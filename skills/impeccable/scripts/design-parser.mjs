...
###[^
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!/^The.*(Rule|Fallback|Principle)/i.test(headerName)) continue;
...
+/g, ' ')).trim();
  ...
  for (const b of collectBullets(lines)) {
    ...
    if (!/^The.+(Rule|Fallback|Principle)$/i.test(nameRaw)) continue;
...
function extractColors(section) {
  ...
  const groups = [];
  const ROLE_KEYWORDS = /^(primary|secondary|tertiary|neutral|accent)/i;
...
function extractInlineShadows(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    const nameMatch = before.match(/([A-Za-z][A-Za-z\- ]{2,40})\s+shadow[^A-Za-z0-9]*$/i);
    let name = null;
    if (nameMatch) {
...
function parseShadowBullet(bullet) {
  ...
  const looksLikeShadow =
    /box-shadow|rgba?\(|px|rem|^-?\d+\s/i.test(rawValue) &&
    /\d/.test(rawValue);
